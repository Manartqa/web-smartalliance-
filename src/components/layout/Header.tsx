    ...
    <header className="sticky top-0 z-50 border-b border-line bg-white">
      <div className="container-site flex h-16 items-center justify-between gap-4 lg:h-20">
        ...
        <nav className="hidden items-center gap-8 lg:flex">
          {navItems.map((item) => (
            <Link
              ...
              className={`relative py-1 text-sm uppercase tracking-[0.02em] transition-colors ${
                isActive(item.href)
        ...
        <div className="hidden items-center gap-5 lg:flex">
          ...
          <Link
            href="/contact"
            className="inline-flex items-center gap-2.5 rounded-[10px] bg-yellow px-5 py-3.5 text-[13px] font-semibold uppercase leading-none tracking-[0.4px] text-[#0a0f1a] transition-colors hover:bg-[#e6b000]"
          >
            {t("contactCta")}
            <ChevronRight />
          </Link>
        ...
        <nav className="container-site flex flex-col py-2">
          {navItems.map((item) => (
            <Link
              ...
              aria-current={isActive(item.href) ? "page" : undefined}
              className={`border-b border-line/70 py-4 text-base ${
                isActive(item.href)
